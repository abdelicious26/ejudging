...
function CreateEvent() {
    ...
    const onSubmit = (e) => {
        ...
        if (scoringType !== 'Ranking') {
            let _totalPercent = 0;
            let _percentError = false
            saveCriteria.forEach(res => {
                console.log('loop criteria', res.percent);
                if (res.percent < 1 || res.percent > 100) {
                    console.log('error percent', res.percent);
                    isError = true;
                    _percentError = true;
                }
